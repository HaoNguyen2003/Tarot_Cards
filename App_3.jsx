...
/**
 * App = ImageScrollGallery (Click-to-Random-Spin + auto-load ảnh từ GitHub)
 * ------------------------------------------------------------------
 * - Danh sách ảnh được lấy TỰ ĐỘNG từ GitHub Contents API, không cần
 *   biết trước có bao nhiêu ảnh hay đặt tên file theo quy tắc A-Z.
 * - Click vào canvas -> chạy random qua nhiều ảnh (hiệu ứng slot machine)
 *   rồi chậm dần, dừng ở 1 ảnh ngẫu nhiên.
...
function buildStepDurations(totalSteps, minDur = 60, maxDur = 650) {
  ...
  for (let i = 0; i < totalSteps; i++) {
    ...
    const eased = t * t; // chậm dần về cuối
...
function App() {
  ...
  }, [githubOwner, githubRepo, githubPath, githubBranch, imageSize]);
